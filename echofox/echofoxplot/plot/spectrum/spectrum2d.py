...
def plot2d(spec,
    ...
    if markers_f2 is None: markers_f2 = []
    ...
    if f2_ticklabel_kwargs is None: f2_ticklabel_kwargs = {}
    if legend_kwargs is None: legend_kwargs = {}
    if mark_peaks_kwargs is None: mark_peaks_kwargs = {}
    if pick_peaks_kwargs is None: pick_peaks_kwargs = {}
    ...
    if show_f1_axis and show_f1_label:
        ...
        ax.set2D_f1_label_text(f1_label_text, pos=pos_f1, f1_label_kwargs=f1_label_kwargs,
                               f1_ticklabel_kwargs=f1_ticklabel_kwargs)

    if show_f2_axis and show_f2_label:
        ax.set2D_f2_label_text(f2_label_text, pos=pos_f2, f2_label_kwargs=f2_label_kwargs,
    ...
    # add projections
    if projections is not None: _add_projections(spectra, projections, ax, f1_proj_ax, f2_proj_ax)

    # add traces
    if traces is not None: _add_traces(spectra, traces, ax)
    ...
    if insets is not None:
        for inset in insets:
            ...
            plot2d(spectra, current_figure=figure, current_ax=inset_ax, ppm_range=excerpt, show_f1_axis=False,
                   show_f2_axis=False, **{**kwargs, **ax_kwargs})
...
def _add_projections(spectra, projections, ax, f1_proj_ax, f2_proj_ax):
    for projection in projections:
        ...
        for i, spectrum in enumerate(spectra):
            if i in where:
                plot_ax.plot_1d_trace_or_projection_from_2d_spectrum(spectrum, trace_ppm=trace_ppm, axis=axis,
                                                                     **projection_kwargs[i])
...
def _mark_peaks(spectrum, ax, mark_peaks_kwargs):
    ...
    x_val, y_val, marker_size_f1_ppm, marker_size_f2_ppm = zip(*[
        (peak.chemical_shifts[1].ppm, peak.chemical_shifts[0].ppm, peak.linewidths[0] / peak.frequencies[0],
         peak.linewidths[1] / peak.frequencies[1]) for peak in peaks])
    ...
    for x, y, w, h in zip(x_val, y_val, marker_size_f2_ppm, marker_size_f1_ppm):
        ...
        rect = Rectangle((x - (w + scale) / 2, y - (h + scale2) / 2), w + scale, h + scale2,
                         facecolor='none',
                         edgecolor=mark_peaks_kwargs['edgecolor'], linewidth=0.5)
        ax.add_patch(rect)
...
def _draw_peaks_from_list(peaks, ax, group):  # for usage in GUI without a need for new picking
    if not peaks is None and len(peaks) > 0:
        if len(peaks[0]) < 8:
            ...
            x_val, y_val, marker_size_f1_ppm, marker_size_f2_ppm = zip(*[(a[2], a[1], a[7], a[8]) for a in peaks])
