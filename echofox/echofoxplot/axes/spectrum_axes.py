...
class SpectrumAxes(Axes):
    ...
    def plot1d(self, spectrum: NmrSpectrum,
               spectrum_color: Union[
                   str, tuple[float, float, float], tuple[float, float, float, float], None] = None,
               f1_offset: float=0,
        ...
        if spectrum.data.ndim == 1:
            self.plot(spectrum.get_ppm_axis(0) + f1_offset, (spectrum.data * intensity_scale) + intensity_offset,
                              **kwargs)
        elif spectrum.data.ndim == 2: # treat as pseudo 2D
            self.plot(spectrum.get_ppm_axis(1) + f1_offset, (spectrum.data[index] * intensity_scale) + intensity_offset,
                      **kwargs)
    ...
    def plot2d(self, spectrum: NmrSpectrum, *args, min_sino: Union[int, float, None] = None,
               ...
               contour_color_negative: Union[
                   str, tuple[float, float, float], tuple[float, float, float, float], None] = None,
               contour_num: int = config.default_contour_levels, contour_factor: float = config.contour_level_factor,
    ...
    def plot_1d_trace_or_projection_from_2d_spectrum(self, spectrum: NmrSpectrum, trace_ppm: str, *args,
        ...
        if shift_to_edge:
            if trace_ppm == 'projection_internal':
                ...
                if axis == 'f1':
                    offset = float(trace_ppm.replace('ppm', '')) * -1 + max(self.get_ylim()) - (
                            max(self.get_ylim()) - min(self.get_ylim())) * 0.01
                elif axis == 'f2':
                    offset = float(trace_ppm.replace('ppm', '')) * -1 + min(self.get_xlim()) + (
        ...
        if not 'color' in kwargs.keys():
            ...
            for ax in axes:
                ...
                plotted_spectrum = [plotted_spectrum for plotted_spectrum in ax.spectra if
                                    plotted_spectrum[0] == spectrum]
                if plotted_spectrum:
        ...
        if axis == 'f1':
            if trace_ppm == 'projection_internal':
                ...
                min_trace, max_trace = min(trace[start_index:end_index]), max(trace[start_index:end_index])
                ...
                if rel_scale:
                    trace = float(trace_ppm.replace('ppm', '')) + offset - ((spectrum.get_row(trace_ppm) / np.max(spectrum.data)) * rel_scale)
                else:
                    trace = float(trace_ppm.replace('ppm', '')) + offset - ((spectrum.get_row(trace_ppm)) * abs_scale)

            else:
                raise Exception(
                    """Trace must be either 'projection' or chemical shift in [ppm] e.g '9.8 ppm' .""", 106)

            self.plot(spectrum.get_ppm_axis(1)[start_index:end_index], trace[start_index:end_index], *args, **kwargs)
            ...
            if trace_ppm == 'projection_internal':
                ...
                if rel_scale:
                    trace = ((spectrum.get_column(trace_ppm) / np.max(spectrum.data)) * rel_scale) + float(trace_ppm.replace('ppm', '')) + offset
                else:
                    trace = ((spectrum.get_column(trace_ppm)) * abs_scale) + float(trace_ppm.replace('ppm', '')) + offset
            else:
                raise Exception(
                    """Trace must be either 'projection' or chemical shift in [ppm] e.g '9.8 ppm' .""", 106)

            self.plot(trace[start_index:end_index], spectrum.get_ppm_axis(0)[start_index:end_index], *args, **kwargs)
    ...
    def set1D_f1_label_text(self, label: str | None = None, pos: tuple[float, float] | None = None,
                            f1_label_kwargs: dict = None, f1_ticklabel_kwargs: dict = None):
    ...
    def set1D_intensity_label_text(self, label: str | None = None, pos: tuple[float, float] | None = None,
                            intensity_label_kwargs: dict = None, intensity_ticklabel_kwargs: dict = None):
    ...
    def set2D_f1_label_text(self, label: str | None = None, pos: tuple[float, float] | None = None,
                            f1_label_kwargs: dict = None, f1_ticklabel_kwargs: dict = None,  debug=False):
        ...
        if label is None: label = self.spectra[0][0].f1_label_text
        ...
        if pos is not None: self.f1.set_label_coords(*pos)
        ...
        label_bbox = self.f1.label.get_window_extent(self.figure.canvas.get_renderer()).transformed(self.figure.transFigure.inverted())

        # Check against tick labels
        for i, tick in enumerate(ticks):
            if tick.get_text():
                tick_bbox = tick.get_window_extent(self.figure.canvas.get_renderer()).transformed(self.get_figure().transFigure.inverted())
                if self._bbox_overlap(tick_bbox,label_bbox):
                    ticklabels[i] = ''

        box = self.spines['left'].get_window_extent(self.figure.canvas.get_renderer()).transformed(self.get_figure().transFigure.inverted())
    ...
    def set2D_f2_label_text(self, label: str | None = None, pos: tuple[float, float] | None = None,
                            f2_label_kwargs: dict = None, f2_ticklabel_kwargs: dict = None) -> None:

        if f2_label_kwargs is None: f2_label_kwargs = {}
    ...
    def _plot_bbox_figure(self, fig, bbox, color='r', label="BBox"):
        ...
        rect = patches.Rectangle((x0, y0), width, height, transform=fig.transFigure,
    ...
    def add_external_projection(self,axis='f1', scale_x=0.85, scale_y=0.85, **kwargs):
        ...
        if axis == 'f1':
            ...
            inset_ax = self.get_figure().add_axes([inset_x0, inset_y0, inset_x1 - inset_x0, inset_y1 - inset_y0],
    ...
    def add_inset(self, excerpt, position, size, **kwargs):
        ...
        # Create a rectangle
        rect = Rectangle((x1, y1), x2 - x1, y2 - y1, edgecolor='black', facecolor=(1, 1, 1, 0.5), linewidth=0.5)
